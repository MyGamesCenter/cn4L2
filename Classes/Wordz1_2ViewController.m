...
/*
  ...
  110804  cloned to cn4L2 -- add back hintCounter handling of score report / calc. halprog 
  Version 3.02:
  110816 bug calculating valid board click -- board too big!!! 
 
  110913 add review mode button image swap.  part way done...  halprog
  110921 begin pause state re-sync fix 110921 
 post 3.05:  if paused, make error sound.
                 [incorrectSound play];                   //110921
 
 */
// 120804 soundOn/Off update 3.90

// 120928 uses new cardback resources, new arrays for image file names and UIImageViews;
//        image file names embedded in code for now


#import <QuartzCore/QuartzCore.h>   // don't miss this! 
...
//		NSLog(@"in setup sound.");
    ...
    turnSoundOnImage =  [UIImage imageNamed: @"turnSoundOn.png"];

    turnSoundOffImage = [UIImage imageNamed: @"turnSoundOff.png"];
...
// get user preferences or set them if they are not saved
    ...
    */
    ...
    kSoundOn = 1; bSoundOn = TRUE;  // 120805
...
//    NSLog(@" language found is %@.", language);
...
//        NSLog(@"We have SPANISH...");
    ...
    // version 3.18 -- chinese UI not supported for Cn4L2
...
//        [k_L1 initWithString:@"DE"];        wrong :-)
    ...
    else if ([language isEqualToString: @"ko"]) {     // Korean is ko :-)
        //        NSLog(@"We have korean...");
...
//  learning french won't be available in FRENCH!
...
120807:   k_L1 has device language, or EN.   version 3.90 
     
 Now, if LLString is NOT default, change k_L1 to match it. unless bogus.
...
*/    
    ...
    hintString  = NSLocalizedString(@"HINT", nil);
    hintString2  = NSLocalizedString(@"HINT2", nil);
...
// halprog version 3.00  -- don't need picturelang, L2 etc... only L1 can vary / be overridden.
	//
	if (![[NSUserDefaults standardUserDefaults]
...
//    NSLog(@" K_l1 and LLString are %@ and %@.", k_L1, LLString );
    // make sure valid
...
//        ([LLString isEqualToString: @"ZH"]) ||
    ...
    */
    ...
    // set default  version 3.18
    
//    databaseName = kDatabaseNameL24EN;
//    k_L1 = @"EN";
    
    if (![LLString isEqualToString: @"DEFAULT"]) {  // START LL-NOT-DEFAULT   120805
// only mess with L1 language setting if value is not DEFAULT...
...
//    }
    ...
    //  learning french will be 'words for begginers' -- each app will have one of these for the L1:L1 pair
...
//    [hintSound play];    test new approach 120805 ver 3.90
      [self playSoundIfNonMuted:hintSound];
...
//                    NSLog(@"Left...");
    [self playSoundIfNonMuted:hintSound];  //120805
//                    [hintSound play];
                    [self nextMode];
            break; 
...
if (!hasLoadedDB) {
    ...
    tileBackPicImageNMa[2] = @"cardback_square_100r.png";
    tileBackPicImageNMa[4] = @"cardback_square_100b.png";
    tileBackPicImageNMa[0] = @"cb-butterfly-pic100.png";
    tileBackPicImageNMa[3] = @"cb-daisies-pic100.png";
    tileBackPicImageNMa[5] = @"cb-fish_angel-pic100.png";
    tileBackPicImageNMa[1] = @"cb-ladybug-pic100.png";
    tileBackPicImageNMa[6] = @"cb-rose-pic100.png";
    tileBackPicImageNMa[7] = @"cb-flower-pic100.png";
...
//	NSLog(@"\n\t\tIn giveHint!!!");
    ...
    //  110824  BEGIN
    // Hints for CN from database.    
    // Otherwise, if hints enabled, message is just tileNM and matchingTileNM for the FC tile
    //
    ...
    // here -- if language pair is EN and CN, do original hint.
    
    // otherwise do new hint
...
//    NSLog(@"PictureLange is: %@.", pictureLang);
    ...
    /* NSLOG BLOCK BEGIN
     ...
     NSLog(@"          THis is FC tile Lang ind: %@", tileLangID[fcseqn[fcPointer]]); // change to use fcPtsArrayIDX ?
     NSLog(@"          THis is FC tile Lang dir: %@", tileLangID[fcPtsArrayIDX]); // change to use fcPtsArrayIDX ?
     NSLog(@"        THis is FC tile tileNM: %@", tileNM[fcseqn[fcPointer]]);
     NSLog(@"THis is FC tile matchingTileNM: %@", matchingTileNM[fcseqn[fcPointer]]);

     NSLog(@"THis is TB tile Lang ind: %@", tileLangID[tindex[fcPointer]]);                 // change to use selPointer? 
     NSLog(@"THis is TB tile Lang dir: %@", tileLangID[selPointer]);                 // change to use selPointer?     
    
     NSLog(@"        THis is TB tile tileNM: %@", tileNM[selPointer]);
     NSLog(@"THis is TB tile matchingTileNM: %@", matchingTileNM[selPointer]);  
    ...
    */ 
    ...
    // do not show CN/EN hints in KR learning program ever !! 
    /*
     if ((tileLangID[selPointer] == kLEN)    ||    // fcseqn[fcPointer]
         (tileLangID[fcseqn[fcPointer]] == kLEN))  // either way.. 
     
     { // if (EN/CN language pair)
     
     //        <#statements-if-true#>
     
     NSLog(@" have EN tile: fc %@ and tb %@ ",tileLangID[fcseqn[fcPointer]] ,tileLangID[tindex[fcPointer]]  );
     */   
    // 
    
//    if ([pictureLang isEqualToString: kLEN])  {
        
    if ([k_L1 isEqualToString: kLEN])  {        
    
//    If (pictureLang == kLEN) then {
        
//       NSLog(@" have EN tile: fc %@ and tb %@ ",tileLangID[fcseqn[fcPointer]] ,tileLangID[tindex[fcPointer]]  );
    
     statusLabel.text = [ NSString stringWithFormat:hintString,  //  @"Hint: >>%@<<",
     // was tileHint[fcPtsArrayIDX  
     ...
     //       
    // end of comment out CNEN hints
//         NSLog(@" have NO ?? EN tile: fc %@ and tb %@ ",tileLangID[fcseqn[fcPointer]] ,tileLangID[tindex[fcPointer]]  );
    
       
    
    //        load up tileNM, matchingTileNM as hint.
    statusLabel.text = [ NSString stringWithFormat: hintString2,  //  
                        // was tileHint[fcPtsArrayIDX  
                        tileNM[fcPtsArrayIDX], matchingTileNM[fcPtsArrayIDX]
                        ];
    
    
    
     }  // due to CN?EN HINT COMMENT BLOCK
    
    
    
    /*  moved above.
     
     statusLabel.text = [ NSString stringWithFormat: @"Hint: >>%@<<",  //  
...
- (IBAction)toggleSound  {
	...
	if (bSoundOn == 1) {
		kSoundOn = 0;
        bSoundOn = FALSE;
         [soundButton setImage: turnSoundOnImage forState:(UIControlStateNormal) ];
        
        // fix mute button // 120805
...
//        [pauseButton setImage: resumeImage forState:(UIControlStateNormal) ];        // 110913
		...
		kSoundOn = 1;
        bSoundOn = TRUE;
         [soundButton setImage: turnSoundOffImage forState:(UIControlStateNormal) ];
        // fix mute button //120805
        //		[pauseButton setTitle: pauseString forState:(UIControlStateNormal)];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
	...
	if (isPlaying) {
		...
		// if not a tile, ignore here i- check if in the zone...
...
//                [incorrectSound play];                   //110921
                [self playSoundIfNonMuted:incorrectSound];  //120805
			} else {
				
			[self tileClicked:IndX  withY:IndY];	
			}
			
		} // end if on tile board
	}  // end if isPlaying 
} // end touches begine

//-(void)flipTileInPlace:(int)tfX withY:(int)tfY;   

/* 
 moreed in favor of simplue UIKit flip animation...

-(void)flipTileInPlace:(int)tfX withY:(int)tfY {   //110903 added for dual layer flip
    // do flip here.
    // after this routine, image will be replaced with back.
    //
    int sPtr; 
    
//    NSLog(@"New flip routine..... for tile %d, %d.", tfX, tfY); 
    
    // things in eed to use:  image of tile, image of back.
    // 
    sPtr = tilePointer[tfX][tfY]; 
...
//    
 ...
 //
    ...
    if  ([tileLangID[sPtr] isEqualToString: k_L1]  ) { // two NSStrings equal? 
        
//    NSLog(@"so will flip over the %@ image to the tileback image.", tileWordPicImageNM[sPtr] );
    } else {
//    NSLog(@"so will flip over the %@ image to the tileback image.", tilePicPicImageNM[sPtr] );        
    }
    
    
    
    
//   110903 BEGIN setup for flip, then do it.
//	isFlipped = NO;
...
//    setImage:[UIImage imageNamed:tileBackPicImageNM]
    
    [UIView transitionWithView:tiles[tfX][tfY] duration:0.3 options:UIViewAnimationOptionTransitionFlipFromLeft 
                    animations:^{  
                        [tiles[tfX][tfY] setImage:[UIImage imageNamed:tileBackPicImageNM]];
                    }
                    completion:NULL];     
...
*/
    ...
    // 110903 modified to use fileTileInPlace routine for animated flip...
...
//    NSLog(@"The name of the picture in this tile should be: %@", tileSaves[indexX][indexY]);
	...
	*/
	...
	if (tileState[indexX][indexY] == 1) // if tile not flipped  aka face up aka State 1  (set to state 0 at... 
	{
...
// time to FLIP 110903 FLIP FLIP FLIP  110903
            // FLIP NOW
            
            
				 tileState[indexX][indexY] = 0;  // flip state to zero from 1 -- IF MATCH
//				 [correctSound play];
                 [self playSoundIfNonMuted:correctSound];  //120805
			
//			     110309  flip if alpha = 0; fad if alpha = 1.
			if (useAlpha == 0) {
				//flip with animation
                
//                [self flipTileInPlace:indexX withY:indexY];   ///halprog
                
                // 			[self tileClicked:IndX  withY:IndY];	

                
              // line below commented out in favor of call of flipTileInPlace ...
                
                
			//		[tiles[indexX][indexY] setImage:[UIImage imageNamed:tileBackPicImageNM]];
                // begin flip v3.05  -- 120929 
                [UIView transitionWithView:tiles[indexX][indexY] duration:flipDuration options:UIViewAnimationOptionTransitionFlipFromLeft
                                animations:^{
                                    [tiles[indexX][indexY] setImage:[UIImage imageNamed:tileBackPicImageNMa[imageBackIDX]]];
                                }
                                completion:NULL];
                // end flip v3.05
			} else {
...
// 110804 check for hint counter and use in scores.... since CN4EN only instance where this can happen.....            
...
//            NSLog(@"________you win!!! _____AA_______________");	// halprog 110725
		...
		// begin part two // ======================================================================================================
            ...
            NSString *scoreString = [[NSString alloc] initWithContentsOfFile:self.highScoreFilename encoding:NSUnicodeStringEncoding error:&error];            
...
//			[winnerSound play];
            [self playSoundIfNonMuted:winnerSound];  //120805
...
//		NSLog(@"Now in winner routin with existing file?? not sure.. ");
        ...
        NSError *error = nil; // new line inserted for use in next line oop.s try NSUnicodeStringEncoding
...
//		NSLog(@"ready to write file.... hal 100820.");
		...
		*/ 
//		[highScoreSound play];
        [self playSoundIfNonMuted:highScoreSound];  //120805
...
- (void)playSoundIfNonMuted:(SoundEffect* ) soundName {
